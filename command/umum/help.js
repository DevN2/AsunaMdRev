...
module.exports = {
	...
	async run({ msg, conn }, { q, owner, map, args }) {
		...
		「 Time 」							「 Speed 」
- ${moment.tz(config.timezone).format("HH:mm:ss")}		- ${processTime(msg.messageTimestamp, moment())} seconds
									「 Date 」
...
hydratedTemplate: {
					...
					hydratedFooterText: "© Asuna • Bot", 
					hydratedButtons: [
						{ quickReplyButton: { displayText: "Ping 📶", id: "#p" } },
